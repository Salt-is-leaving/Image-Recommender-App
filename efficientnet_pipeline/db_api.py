...
def update_feature_metadata(conn, image_id, has_hsv=None, has_efficientnet=None):
    ...
    try:
        ...
        cursor.execute("""
            SELECT 
                ...
                SUM(has_hsv AND has_efficientnet) as complete_features
...
def get_images_with_complete_features(conn):
    ...
    try:
        ...
        cursor.execute("""
            ...
            WHERE f.has_hsv = 1 AND f.has_efficientnet = 1
        """)
        return cursor.fetchall()
    except sqlite3.Error as e:
...
def combine_all_features(image_ids, normalize_features=True):
    ...
    combined_features = {}
    feature_stats = {'efficientnet': 0, 'hsv': 0}
...
def get_weighted_similarity(target_features, all_features, weights=None):
    ...
    if weights is None:
        weights = {'efficientnet': 0.5, 'hsv': 0.2}

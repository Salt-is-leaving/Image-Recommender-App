...
from config import PATH_TO_SSD, find_image_in_database, get_similarity_weights
...
class ClusteringFirstSearch:
    ...
    def create_compatibility_indices(self):
        ...
        for feature_type, features in self.features_data.items():
            ...
            common_paths = list(set(features.keys()) & set(cluster_assignments.keys()))
    ...
    def build_indices(self):
        ...
        for feature_type in self.features_data:
            ...
            for image_path, cluster_id in assignments.items():
            ...
            for cluster_id, cluster_features in cluster_groups.items():
                ...
                try:
                    ...
                    feature_indices[cluster_id] = {
                        'index': index,
                        'image_paths': cluster_image_paths[cluster_id],
                        'features': feature_matrix
                    }
                except Exception:
    ...
    def search_within_clusters(self, query_features, feature_type, top_k=50):
        """Search within relevant clusters with better error handling."""
        try:
            ...
            for cluster_info in cluster_assignments[feature_type]:
                ...
                try:
                    if FAISS_AVAILABLE and index is not None:
                        # FAISS search
                        search_k = min(top_k, len(image_paths))
                        ...
                        all_results.extend(
                            self.direct_cluster_search(query_vector[0], feature_matrix, 
                                                      image_paths, feature_type, cluster_id)
                        )
    ...
    def direct_cluster_search(self, query_flat, feature_matrix, image_paths, feature_type, cluster_id):
    ...
    def fallback_direct_search(self, query_features, feature_type, top_k):
        ...
        results = []
        for image_path in valid_ids[:1000]:  # Limit for performance
...
def comparison_mode(target_image_path=None, use_gpu=False, compare_all_methods=True, enable_clustering=True):
    ...
    if target_image_path not in available_images:
        logging.error(f"Target image {target_image_path} not found")
        return False
    ...
    if 'hsv' in individual_rankings:
        ...
        if hsv_rankings:
            ...
            integrated_ids = {r['image_paths'] for r in integrated_results[:5]}
